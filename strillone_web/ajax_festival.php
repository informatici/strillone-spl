...
if (isset($_POST["speech"]) && ($_POST["speech"] != '')) {
	...
	if ($speech != "") {
        ...
        $speech = preg_replace('/(\d+)\'\'/', '${1}s', $speech);
        $speech = preg_replace('/(\d+)\'(\d+s)?/', '${1}m ${2}', $speech);
        trace($speech);
		...
		if (file_exists($speech_file)) {
			...
			$text2wave_cmd = sprintf("text2wave -eval \"(voice_pc_mbrola) (Parameter.set 'Duration_Stretch 0.70)\" -o %s -scale %d %s",$wave_file,$volume_scale,$speech_file);
			exec($text2wave_cmd);
